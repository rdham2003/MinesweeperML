...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("AI_play").addEventListener("click", function(event){
        event.preventDefault();
        const aiButton = document.getElementById("AI_play");
        console.log(aiButton);
        let board = minesweeperboard.map(row => [...row]);
        for (let i = 0; i < len; i++){
            for (let j = 0; j < len; j++){
        ...
        console.log('Board sent to AI:', board);
        ...
        fetch('/AI_play', {
            ...
            body: JSON.stringify({
                board_reveal: board,
                board_unreveal: minesweeperboard,
                is_first_move: isFirstMove
            })
        })
        .then(response => {
            if (!response.ok) {
